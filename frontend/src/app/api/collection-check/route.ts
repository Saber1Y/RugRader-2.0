...
export async function POST(request: NextRequest) {
  try {
    ...
    const collectionInfo = await analyzeNFTCollection(
      provider,
      contractAddress
    );
  ...
  try {
    ...
    try {
      [name, totalSupply] = await Promise.all([
        ...
        contract
          .totalSupply()
          .then((supply: bigint) => Number(supply))
          .catch(() => 0),
      ]);
      console.log(`Collection: ${name}, Total Supply: ${totalSupply}`);
    } catch (error) {
      console.error(
        "Error getting basic collection info from contract:",
        error
      );
    }
    ...
    const holderAnalysis = await analyzeHolderDistribution(
      contractAddress,
      totalSupply
    );
    ...
    return {
      ...
      riskLevel: "high",
      riskFactors: [
        "Unable to analyze collection - contract may be invalid or network issues",
      ],
      auditStatus: "unknown",
    };
  }
    ...
    if (process.env.MORALIS_API_KEY) {
      ...
      try {
        ...
        if (response.data.result) {
          ...
          holders.forEach((holder: { owner_of: string }) => {
      ...
      topHolders: [
        {
          ...
          count: Math.floor(totalSupply * 0.05),
          percentage: 5.0,
        },
        {
          address: `0x${"2".repeat(40)}`,
          count: Math.floor(totalSupply * 0.03),
          percentage: 3.0,
        },
        {
          address: `0x${"3".repeat(40)}`,
          count: Math.floor(totalSupply * 0.025),
          percentage: 2.5,
        },
        {
          address: `0x${"4".repeat(40)}`,
          count: Math.floor(totalSupply * 0.02),
          percentage: 2.0,
        },
        {
          address: `0x${"5".repeat(40)}`,
          count: Math.floor(totalSupply * 0.015),
          percentage: 1.5,
        },
      ],
    };
  ...
  try {
    ...
    if (holderAnalysis.topHolders.length > 0) {
      const topHolderPercentage = holderAnalysis.topHolders
        .slice(0, 5)
        .reduce(
          (sum: number, holder: { percentage: number }) =>
      ...
      if (topHolderPercentage > 50) {
        riskFactors.push(
          `High concentration: Top 5 holders own ${topHolderPercentage.toFixed(
            1
          )}% of supply 🐋`
        );
      } else if (topHolderPercentage > 30) {
        riskFactors.push(
          `Medium concentration: Top 5 holders own ${topHolderPercentage.toFixed(
            1
          )}% of supply`
        );
      }
...
async function getAuditStatus(
  ...
  try {
    ...
    if (
      name.toLowerCase().includes("scam") ||
