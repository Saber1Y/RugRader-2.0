...
export default function Web3AnalyzerPage() {
  ...
  const formatWalletResult = (data: WalletResult) => {
    ...
    resultText += `ETH Balance: ${parseFloat(data.ethBalance).toFixed(
      4
    )} ETH\n`;
    ...
    if (data.tokens.length > 0) {
      ...
      data.tokens.forEach((token: WalletResult["tokens"][number]) => {
        resultText += `• ${token.symbol}: ${parseFloat(token.balance).toFixed(
          2
        )}`;
        if (token.price) {
          resultText += ` ($${(parseFloat(token.balance) * token.price).toFixed(
            2
          )})`;
        }
    ...
    if (data.nfts.length > 0) {
      ...
      data.nfts.forEach((nft: WalletResult["nfts"][number]) => {
  ...
  const formatCollectionResult = (data: CollectionResult) => {
    ...
    if (data.topHolders.length > 0) {
      ...
      data.topHolders
        .slice(0, 5)
        .forEach(
          (holder: CollectionResult["topHolders"][number], index: number) => {
            resultText += `${index + 1}. ${holder.address.slice(
              0,
              6
  ...
  const formatNFTResult = (data: NFTResult) => {
    ...
    if (data.metadata) {
      ...
      if (data.metadata.owner) {
        resultText += `Owner: ${data.metadata.owner.slice(
          0,
          6
